...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden.weight = nn.init.xavier_uniform_(tensor=self.embed_to_hidden.weight,gain=1)
        self.dropout = nn.Dropout()
        self.hidden_to_logits = nn.Linear(self.hidden_size, self.n_classes)
        self.hidden_to_logits.weight = nn.init.xavier_uniform_(tensor=self.hidden_to_logits.weight,gain=1)
        ### END CODE HERE
    ...
    def forward(self, t):
        ...
        embedding_lookup = self.embedding_lookup(t)
        embed_to_hidden = self.embed_to_hidden(embedding_lookup)
        embed_to_hidden = nn.functional.relu6(embed_to_hidden)
        dropout = self.dropout(embed_to_hidden)
