...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  num_rows, num_cols = outside_vectors.shape
  dot_products = np.zeros(num_rows)

  # create distributions
  for word in outside_vectors:
    i, j = np.where(outside_vectors == word)
    dot_products[i] = np.dot(word, center_word_vec)
  probability_distribution = softmax(dot_products)

  # compute loss
  loss = -np.log(probability_distribution[outside_word_idx])

  # compute center vec gradiant
  true_distribution = np.zeros(num_rows)
  true_distribution[outside_word_idx] = 1
  grad_center_vec = np.dot((probability_distribution - true_distribution), outside_vectors)
  # compute outside vec gradiant
  grad_outside_vecs = np.dot(np.atleast_2d(probability_distribution - true_distribution).T, np.atleast_2d(center_word_vec))
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for word in outside_words:
    loss_current, gradc, grado = word2vec_loss_and_gradient(center_word_vectors[word2ind[current_center_word]], word2ind[word], outside_vectors, dataset)
    loss += loss_current
    grad_center_vecs[word2ind[current_center_word]] += gradc
    grad_outside_vectors += grado
    if counter == 2*window_size:
      break
    counter += 1
...
def sgd(f, x0, step, iterations, postprocessing=None, use_saved=False,PRINT_EVERY=50):
